...
class TargetSpace(object):
    ...
    '''
        ...
        更新时间：2021/1/5  14:15
    '''
    def __init__(self, target_func, pbounds, precisions, random_state=None):
        """
        Parameters
        ----------
    ...
    def random_sample(self):
        ...
        '''
                新增属性 precisions
                方法：根据精度随机生成对应精度的数据（主要有两种精度 1.0 和 0.01 ）
                更新时间：2021/1/5  14:15
        '''
        bounds_and_pre = np.column_stack((self._bounds, self._precisions))

        data = np.empty((1, self.dim))
        for col, (lower, upper, pre) in enumerate(bounds_and_pre):
            if pre == 1.0:
                data.T[col] = np.random.randint(lower, upper ,size=1)
            if pre == 0.01:
                data.T[col] = np.round(self.random_state.uniform(lower, upper, size=1), 2)
        return data.ravel()
