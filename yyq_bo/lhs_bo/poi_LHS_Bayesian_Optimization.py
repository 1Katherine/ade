...
def draw_target(bo):
    # 画图
    plt.plot(-bo.space.target, label='lhs_bo_poi  init_points = ' + str(init_points) + ', n_iter = ' + str(n_iter))
    ...
    plt.ylabel('runtime')
    plt.legend()
    plt.savefig("./2lhs_searching_config_poi/target - " + str(drawtime.strftime( '%Y-%m-%d %H-%M-%S')) + ".png")
    plt.show()
...
if __name__ == '__main__':
    ...
    init_points = 20
    n_iter = 20
    # xi = 0.01 = le-1 : prefer expolration
