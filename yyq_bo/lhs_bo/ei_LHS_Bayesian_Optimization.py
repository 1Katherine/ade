...
def draw_target(bo):
    # 画图
    plt.plot(-bo.space.target, label='lhs_bo_ei  init_points = ' + str(init_points) + ', n_iter = ' + str(n_iter))
    ...
    plt.ylabel('runtime')
    plt.legend()
    plt.savefig("./2lhs_searching_config_ei/target - " + str(drawtime.strftime( '%Y-%m-%d %H-%M-%S')) + ".png")
    plt.show()
...
if __name__ == '__main__':
    ...
    init_points = 20
    n_iter = 20
    # xi = 0.01: prefer expolration            xi = 0 : prefer exploitation
    optimizer.maximize(init_points=init_points, n_iter=n_iter, acq='ei',xi=0.005)
    print('optimizer.max')
    print(optimizer.max)
