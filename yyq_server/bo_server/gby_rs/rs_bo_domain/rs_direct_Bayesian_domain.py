...
print('os.path.dirname(os.path.realpath(__file__)) = ' + str(os.path.dirname(os.path.realpath(__file__))))
from rs_bayes_opt.logger import JSONLogger
from rs_bayes_opt.event import Events
from rs_bayes_opt import BayesianOptimization
from rs_bayes_opt import SequentialDomainReductionTransformer
...
# 服务器运行spark时config文件
config_run_path = "/usr/local/home/yyq/bo/rs_bo/rs_bo_domain/config/" + opts.benchmark +  "/"
# 重要参数
vital_params_path = "/usr/local/home/yyq/bo/rs_bo/rs_bo_domain/parameters_set.txt"
# 维护的参数-范围表
conf_range_table = "/usr/local/home/yyq/bo/rs_bo/rs_bo_domain/Spark_conf_range_"  + opts.benchmark.split('-')[0] +  ".xlsx"
# 保存配置
generation_confs = "/usr/local/home/yyq/bo/rs_bo/rs_bo_domain/generationDomainConf.csv"


def draw_target(bo):
    # 画图
    plt.plot(-bo.space.target, label='rs_bo_domain  init_points = ' + str(init_points) + ', n_iter = ' + str(n_iter))
    ...
    plt.ylabel('runtime')
    plt.legend()
    plt.savefig("/usr/local/home/yyq/bo/rs_bo/rs_bo_domain/target.png")
    plt.show()
...
def run(configNum):
    # configNum = None
    # 使用给定配置运行spark
    run_cmd = '/usr/local/home/zwr/' + opts.benchmark + '-ga.sh ' + str(
        configNum) + ' /usr/local/home/yyq/bo/rs_bo/rs_bo_domain/config/' + opts.benchmark
    os.system(run_cmd)
