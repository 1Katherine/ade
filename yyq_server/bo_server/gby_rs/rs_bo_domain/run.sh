...
mv $path/generationDomainConf.csv $path/config/$1-$startTime
mv $path/target.png $path/config/$1-$startTime

mkdir -p $path/config/$1

python3 $path/rs_direct_Bayesian_domain.py --benchmark=$1 --ninit=$n_initsamples --ninit=$n_interations

finishTime=$(date "+%m-%d-%H-%M")
mv  $path/config/$1 $path/config/$1-$finishTime
mv $path/logs.json $path/config/$1-$finishTime
mv $path/generationDomainConf.csv $path/config/$1-$finishTime
